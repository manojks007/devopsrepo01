$scratch = 'C:\MKS\Terraform_Code'
$devopsmc = 'C:\MKS\Terraform_Code\DevOpsRepo01\DevOpsMC'
...
git config --global user.name "manojks007"
git config --global user.email manojks007@gmail.com
...
git commit -m "scriptfileupdated"
...
#look at the type
git cat-file -t 16a443
#look at the content
git cat-file -p 16a443

#look at the type
git cat-file -t d1bbac
#look at the content. Notice here it points the blob we saw created before and now has a file name
git cat-file -p d1bbac

#look at the type
git cat-file -t 0b7c3
#look at the content. Notice here it points the blob we saw created before and now has a file name
git cat-file -p 0b7c3

git cat-file -t 3c0426
git cat-file -p 3c0426

#Lets prove a point about it only storing unique content
Copy-Item .\testfile01.txt .\testfile02.txt
git add .
#what new object do we have? Nothing.
git status
git commit -m "testfile02.txt added"
#We have new files. Look again

git log --oneline --graph --decorate --all   #I will be using this a lot
git cat-file -t fbe3287  #Note it references the parent commit hash, again, can't change history as hash would not match
git cat-file -p fbe3287
...
code .\testfile01.txt
git add testfile01.txt
git status
#add a 3rd file but don't stage
code .\testfile03.txt
git status

git commit -m "updated testfile01"
git status
...
gitgraph
git diff ff7f419..16a4436 #diff between specific commits
#Remember the complete snapshot is stored. All diffs are generated at time of command execution
git diff fbe3287..d1bbac3
#WB07
git status
git add testfile03.txt
code testfile01.txt #make a change
git status
...
#Example
code testfile05.txt
git add testfile05.txt
git commit -m "adding testfile05.txt"
git status
git rm testfile05.txt
git status #Note the delete is staged
Get-ChildItem #its gone from stage AND my working
git commit -m "removed testfile05.txt"
...
git tag -a v0.0.1 ff7f41 -m "First version"
git show v0.0.1
...
Set-Location $devopsmc
git remote -v

Set-Location $scratch\gitplay1
git remote -v
...
git remote add origin https://github.com/manojks007/DevOpsMC/
#git branch -M main      # RENAMES the branch from master to main
git remote add origin https://github.com/manojks007/

git remote add origin https://github.com/manojks007/mkstestrepo2/
#git branch -M main      # RENAMES the branch from master to main
git remote add origin https://github.com/manojks007/mkstestrepo/gitplay1
...
git push -u https://github.com/manojks007/DevOpsMC/ manojbranch002
...
git remote add origin https://github.com/manojks007/DevOpsMC.git
git push -u origin main
...
git remote add upstream https://github.com/manojks007/JLRepo.git
git remote -v
